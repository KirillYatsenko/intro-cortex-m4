...
static void enable_rows(int8_t rows)
{
	while (rows >= 0)
		enable_row(--rows);
}
...
static void disable_row(uint8_t row)
{
	if (row == 3) {
		// PB4 = row #3
		GPIO_PORTB_DIR_R &= ~0x10;
...
static void disable_rows(int8_t rows)
{
	while (rows >= 0)
		disable_row(--rows);
}
...
static bool scan_col(uint8_t col)
{
	if (col == 0 || col == 1) {
		// PE5 = col 0, PE4 = col 1
...
int matrix_keypad_init(char *str)
{
